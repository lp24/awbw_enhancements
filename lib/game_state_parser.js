...
function parseCoordinatesFromPropertySpan(span) {
    ...
    return {
        ...
        // HQ images cover the square above them too, so compute the "lower square" by offsetting from the bottom
...
class Property {
    ...
    isTower() {
        return ["comtower"].indexOf(this.tile) !== -1;
    }
}
...
class Unit {
    ...
    unitValue() {
        return this.unitData().cost * (parseInt(this.hp || 10) / 10);
    }
}
...
class GameStateParser {
    constructor(gamemap) {
        ...
        this.listeners = [];
        this.weatherCode = kWeatherByCode[scrapeWeatherCode()] || kWeatherClear;
        this.weatherOverride = null; // Manual weather override
    }
    ...
    parseMapEntities() {
        ...
        mapEntities.weather = this.weatherOverride || this.weatherCode;
