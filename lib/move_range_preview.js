    ...
    onClick(event) {
        if (event.target.id === "move") {
            ...
            if (this.rangeTiles.length > 0 && this.displayingSelectedUnit) {
                ...
                this.currentUnit = this.getUnitWithSpan(unitSpan);
                this.showMoveRangeFor(this.currentUnit, /*selected=*/true);
            }
        } else {
            if (event.target.id === "set-hp") {
                document.dispatchEvent(new CustomEvent("playerpanel:updateCharge", {
                    detail: {
                        unit: this.currentUnit,
                        newHp: this.currentUnit.hp || '10'
                    }
                }));
            }
